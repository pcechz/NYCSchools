...
public abstract class NetworkBoundSchoolResource<T, V> {

        private final MediatorLiveData<Resource<T>> result = new MediatorLiveData<>();

        @MainThread
        protected NetworkBoundSchoolResource() {
        ...
        private void fetchFromNetwork(final LiveData<T> dbSource) {
            ...
            createCall().enqueue(new Callback<List<SchoolEntity>>() {
                @Override
                public void onResponse(@NonNull Call<List<SchoolEntity>> call, @NonNull Response<List<SchoolEntity>> response) {
                    result.removeSource(dbSource);
                    saveResultAndReInit(response.body());
                }

                @Override
                public void onFailure(@NonNull Call<List<SchoolEntity>> call, @NonNull Throwable t) {
                    result.removeSource(dbSource);
        ...
        @MainThread
        private void saveResultAndReInit(List<SchoolEntity> response) {
        ...
        @WorkerThread
        protected abstract void saveCallResult(List<SchoolEntity> item);
        ...
        @NonNull
        @MainThread
        protected abstract Call<List<SchoolEntity>> createCall();
